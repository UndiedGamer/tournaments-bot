...
export async function handleChatInputOrError(error: Error, { command, interaction }: ChatInputCommandErrorPayload | ContextMenuCommandErrorPayload) {
	...
	try {
		await alert(interaction, generateUnexpectedErrorMessage());
	} catch (err) {
...
function argumentError(interaction: CommandInteraction, error: ArgumentError<unknown>) {
	return alert(
		interaction,
		error.message ||
			'An error occurred that I was not able to identify. Please try again. If the issue keeps showing up, you should get in touch with the developer'
	);
}
...
function userError(interaction: CommandInteraction, error: UserError) {
	...
	return alert(
		interaction,
		error.message ||
			'An error occurred that I was not able to identify. Please try again. If the issue keeps showing up, you can get in touch with the developer'
	);
}
